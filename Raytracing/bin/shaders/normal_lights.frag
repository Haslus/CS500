...
float shapeSDF(vec3 pos,int index)
{
	...
	vec3 pos = inverse(M2W) * ( pos - shapes[index].position) ;
...
vec2 castRay(vec3 rayOrigin, vec3 rayDir)
{
	...
	vec2 tb = iBox( rayOrigin, rayDir, vec3(10,10,10) );
...
{           
  ...
  // float dist = raymarch(rayOrigin, rayDir);
   ...
   vec3 rdx = viewDir * normalize( vec3(px,2.5) );
   vec3 rdy = viewDir * normalize( vec3(py,2.5) );
